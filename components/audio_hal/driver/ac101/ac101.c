...
{
	// ESP_LOGI("AC101 driver", "Set earph volumm %d", volume);
	uint16_t res,tmp;
	esp_err_t ret;
	res = AC101_read_Reg(HPOUT_CTRL);
	tmp = ~(0x3f<<4);
...
esp_err_t AC101_start(ac_module_t mode)
{
    ...
    if (mode == AC_MODULE_DAC || mode == AC_MODULE_ADC_DAC || mode == AC_MODULE_LINE) {
		...
		AC101_pa_power(0);	// HuyTV
		vTaskDelay(1);

    	//* Enable Speaker output
		res |= AC101_Write_Reg(SPKOUT_CTRL, 0xeabd);
		vTaskDelay(10 / portTICK_PERIOD_MS);
		AC101_pa_power(1);	// HuyTV
		AC101_set_voice_volume(100);
    }
...
esp_err_t AC101_set_speaker_output(bool en)
{
	if(en == 0)
	{
		return AC101_Write_Reg(SPKOUT_CTRL, 0xe880);		//disable speaker
	}
	return AC101_Write_Reg(SPKOUT_CTRL, 0xeabd);		//disable speaker
}


esp_err_t AC101_set_voice_volume(int volume)
{
	// Bug in AC101 driver, fixed by HuyTV
	if(volume > 100) volume = 100;
	volume =  volume * 63 /100;		// 63 is magic numer
	// end
	...
	if(res != ESP_OK)
	{
		ESP_LOGE("AC101", "Cannot set ear volumm, err code %d", res);
	}
	res |= ac101_set_spk_volume(volume);
	if(res != ESP_OK)
	{
		ESP_LOGE("AC101", "Cannot set spk volumm, err code %d", res);
	}
	return res;
}
